...
describe('interceptors', () => {
  ...
  test('should add a response interceptor', done => {
    ...
    getAjaxRequest().then(request => {
      ...
      setTimeout(() => {
        expect(response.data).toBe('See - modified by interceptor')
        done()
      }, 100)
    })
  })
  ...
  test('should add a reponse interceptor that return a promise', done => {
    let response: AxiosResponse
    ...
    getAjaxRequest().then(request => {
      ...
      setTimeout(() => {
        expect(response.data).toBe('you got promised')
        done()
      }, 100)
    })
  })
